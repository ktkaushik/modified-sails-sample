	...
	return {
			...
			if ( sails.config.controllers.blueprints.actions ) {
				...
				_.each(sails.controllers, function (controller, controllerId) {
					...
					_.each(controllerActions, function (actionId) {
						...
						// Bind index route
						if (actionId === 'index') {
							sails.log.error('*************** actionId is Index *******************');
		...
		bindCRUDRoutes: function ( ) {
			...
			_.each(sails.controllers, function (controller, controllerId) {
				...
				pluralControllerId = pluralize.inflect(controllerId);
				// TODO: support blueprint config on a per-controller basis
				if (!util.isDictionary(controller)) {
					return;
				}
				...
				if ( sails.config.controllers.blueprints.rest ) {
					...
					sails.router.bind('get ' + blueprintsPrefix + '/'+ pluralControllerId + '/:id?', _serveBlueprint(controllerId, 'find'));
					sails.router.bind('post ' + blueprintsPrefix + '/'+ pluralControllerId, _serveBlueprint(controllerId, 'create'));
					sails.router.bind('put ' + blueprintsPrefix + '/'+ pluralControllerId + '/:id?', _serveBlueprint(controllerId, 'update'));
					sails.router.bind('delete ' + blueprintsPrefix + '/'+ pluralControllerId + '/:id?', _serveBlueprint(controllerId, 'destroy'));
				}
				...
				if ( sails.config.controllers.blueprints.shortcuts ) {
					...
					sails.router.bind(blueprintsPrefix + '/'+ pluralControllerId + '/find/:id?', _serveBlueprint(controllerId, 'find'));
					sails.router.bind(blueprintsPrefix + '/'+ pluralControllerId + '/create', _serveBlueprint(controllerId, 'create'));
					sails.router.bind(blueprintsPrefix + '/'+ pluralControllerId + '/update/:id?', _serveBlueprint(controllerId, 'update'));
					sails.router.bind(blueprintsPrefix + '/'+ pluralControllerId + '/destroy/:id?', _serveBlueprint(controllerId, 'destroy'));
				}
			});
		...
		loadMiddleware: function(cb) {
			...
			async.auto({
				...
				controllers: [function(cb) {
					...
					sails.log.warn("******* sails.controllers ********");
					sails.log.warn(sails.controllers);
					_.each(sails.controllers, function(controller, controllerId) {
